...
fn build_full(per_payload_len: usize, tag: &str, idx: usize) -> Vec<u8> {
    ...
    let pws: Vec<String> = (0..11).map(|i| format!("ctrl-{tag}-{idx}-pw-{i}")).collect();
    let mut pt = vec![0u8; per_payload_len];
    for i in 0..11 {
        fill_payload(&mut pt, 0x9E37_79B9_7F4A_7C15u64 ^ ((idx as u64) << 32) ^ i as u64 | 1);
        ...
        c.write(&pws[i], &pt, &known, 64, None).expect("write payload");
    }
    c.into_bytes()
}
